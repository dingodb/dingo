...
use tantivy::schema::{
    IndexRecordOption, Schema, TextFieldIndexing, TextOptions, FAST, INDEXED, STORED, TEXT,
};
...
fn build_index_in_ram(schema: Schema) -> Index {
    let text = schema
        .get_field("text")
        .expect("schema doesn't have `text`.");
    let row_id = schema
        .get_field("row_id")
        .expect("schema doesn't have `row_id`.");

    let index = Index::create_in_ram(schema);
    let mut index_writer = index.writer_with_num_threads(2, 1024 * 1024 * 32).unwrap();
...
fn regex_search_demo() {
    ...
    let searcher = build_index_in_ram(schema.clone())
        .reader()
        .unwrap()
        .searcher();
    ...
    let cat_result = searcher.search(&cat, &Count).expect("failed to search");
