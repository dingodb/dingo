...
fn main() {
    ...
    let text_query = query_parser.parse_query("(\"Ancient empires\" AND rise) OR \"balance of power\"").expect("Can't parse query.");
    let result = searcher.search(&text_query, &Count).expect("failed to search");
