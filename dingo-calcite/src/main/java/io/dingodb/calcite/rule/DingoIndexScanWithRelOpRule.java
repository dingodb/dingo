...
public class DingoIndexScanWithRelOpRule  extends ConverterRule {
    public static final Config DEFAULT = Config.INSTANCE
        .withConversion(
            LogicalIndexScanWithRelOp.class,
            Convention.NONE,
            DingoConvention.INSTANCE,
            "DingoIndexScanWithRelOpRule"
        )
        .withRuleFactory(DingoIndexScanWithRelOpRule::new);

    protected DingoIndexScanWithRelOpRule(Config config) {
        super(config);
    }

    @Override
    public @Nullable RelNode convert(RelNode relNode) {
        LogicalIndexScanWithRelOp scan = (LogicalIndexScanWithRelOp) relNode;
