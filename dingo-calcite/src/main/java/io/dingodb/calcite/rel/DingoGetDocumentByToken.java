...
public class DingoGetDocumentByToken extends Filter implements DingoRel {
    @Getter
    ...
    public DingoGetDocumentByToken(RelOptCluster cluster, RelTraitSet traits,
                                   RelNode child,
                                   RexNode condition,
                                   RelOptTable table,
    ...
    public RelDataType getTableType() {
        ...
        builder.add(new RelDataTypeFieldImpl(
            indexTable.getName() + "$token",
            relDataType.getFieldCount(),
            getCluster().getTypeFactory().createSqlType(SqlTypeName.get("String"))));
