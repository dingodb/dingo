...
public class DingoRelOptTable extends Prepare.AbstractPreparingTable {
    ...
    public DingoRelOptTable(@NonNull DingoTable table, String user, String host) {
        ...
        this.schemaName =  table.getSchema().getSchemaName();
    ...
    public static DingoRelOptTable getDefault(RelOptCluster cluster) {
        ...
        names.add("mysql");
        names.add("user");
        DingoTable dingoTable = new DingoTable((DingoParserContext) cluster.getPlanner().getContext(), names, DdlService.root().getIsLatest().getTable("mysql", "user"));
