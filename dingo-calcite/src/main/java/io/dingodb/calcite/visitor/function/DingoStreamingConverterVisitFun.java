...
public final class DingoStreamingConverterVisitFun {
    ...
    public static @NonNull Collection<Vertex> convertStreaming(
        ...
        if (dstPartitions.size() < media.getPartitions().size()) {
            ...
            if (dstDistribution instanceof DingoRelPartitionByIndex &&
                ((DingoRelPartitionByIndex) dstDistribution).getTargetTableList().stream().distinct().toList().size() > 1) {
                return outputs;
            }
    ...
    private static @NonNull Collection<Vertex> hash(
        ...
        if (transaction == null || transaction.isAutoCommit() || visitor.getExecuteVariables().isJoinConcurrency()) {
