...
public class SqlUnLockBlock extends SqlUnLock {
     ...
     * @param pos
     */
    public SqlUnLockBlock(SqlParserPos pos, List<SqlBlock> sqlUnBlockList) {
        super(OPERATOR, pos);
        this.sqlUnBlockList = sqlUnBlockList;
    }

    @Override
    public void unparse(SqlWriter writer, int leftPrec, int rightPrec) {
        writer.keyword("UNLOCK BLOCKS");
        for (int j = 0; j < sqlUnBlockList.size(); j ++) {
            SqlBlock block = sqlUnBlockList.get(j);
