...
public class SqlGenerateSeriesOperator extends SqlFunction implements SqlTableFunction {

    public static void register(DingoParserContext context) {
        StandardConvertletTable.INSTANCE.registerOp(SqlUserDefinedOperators.GENERATE_SERIES,
            (cx, call) -> {
                RexBuilder rexBuilder = cx.getRexBuilder();
                TableGenerateSeriesFunctionNamespace namespace = (TableGenerateSeriesFunctionNamespace) cx.getValidator().getNamespace(call);
                return rexBuilder.makeCall(namespace.getRowType(), call.getOperator(), Collections.EMPTY_LIST);
            });
    }

    public SqlGenerateSeriesOperator(String name, SqlKind kind) {
        super(
            name,
            kind,
            ReturnTypes.ARG0,
