...
import io.dingodb.raft.storage.log.CheckpointFile.Checkpoint;
import io.dingodb.raft.storage.log.SegmentFile.SegmentFileOptions;
...
public class RocksDBSegmentLogStorage extends RocksDBLogStorage {
    ...
    private static final int DEFAULT_CHECKPOINT_INTERVAL_MS = SystemPropertyUtil
        .getInt("jraft.log_storage.segment.checkpoint.interval.ms", 5000);
     ...
     */
    private static final int MAX_SEGMENT_FILE_SIZE = SystemPropertyUtil
        .getInt("jraft.log_storage.segment.max.size.bytes", 1024 * 1024 * 1024);
    ...
    private static int DEFAULT_VALUE_SIZE_THRESHOLD = SystemPropertyUtil
        .getInt("jraft.log_storage.segment.value.threshold.bytes", 4 * 1024);
