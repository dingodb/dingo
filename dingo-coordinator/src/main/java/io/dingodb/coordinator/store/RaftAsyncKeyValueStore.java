...
public class RaftAsyncKeyValueStore implements AsyncKeyValueStore {
    ...
    private RocksRawKVStore rawKVStore;

    private boolean watchWithStack = false;

    public RaftAsyncKeyValueStore() {
        executorService = watchExecutor();
    }

    public void init(CoordinatorContext context) {
        rawKVStore = context.rocksKVStore();
        raftRawKVStore = new RaftRawKVStore(context.node(), rawKVStore, readIndexExecutor());
    ...
    private ThreadPoolExecutor watchExecutor() {
        return new ThreadPoolBuilder()
            .name("Async-kv-executorView-watch")
            .build();
    }
    ...
    public CompletableFuture<Boolean> contains(byte[] key) {
        ...
        CompletableFuture<Boolean> future = doKVOp(closure -> raftRawKVStore.containsKey(key, closure));
        future.thenAcceptAsync(b -> onTouch(key, stack), executorService);
        return future;
    }
    ...
    public CompletableFuture<Boolean> put(byte[] key, byte[] value) {
        ...
        future.thenAcceptAsync(b -> onWrite(key, value, stack), executorService);
        return future;
    }
    ...
    public CompletableFuture<Boolean> delete(byte[] key) {
        ...
        future.thenAcceptAsync(b -> onDelete(key, stack), executorService);
        return future;
    }
    ...
    public CompletableFuture<byte[]> get(byte[] key) {
        String stack = watchWithStack ? StackTraces.stack(2) : null;
        CompletableFuture<byte[]> future = doKVOp(closure -> raftRawKVStore.get(key, closure));
        future.thenAcceptAsync(value -> onRead(key, value, stack), executorService);
        return future;
    }
    ...
    public CompletableFuture<byte[]> getAndPut(byte[] key, byte[] value) {
        ...
        future.thenAcceptAsync(oldV -> onWrite(key, value, stack), executorService);
        future.thenAcceptAsync(oldV -> onRead(key, oldV, stack), executorService);
        return future;
    }
    ...
    public CompletableFuture<byte[]> merge(byte[] key, byte[] value) {
        ...
        future.thenAcceptAsync(b -> this.<byte[]>doKVOp(closure -> raftRawKVStore.get(key, closure)).thenAccept(r -> {
    ...
    public synchronized CompletableFuture<Long> increment(byte[] key) {
        ...
        get(key).whenComplete((r, e) -> {
            if (e == null) {
                if (r == null) {
                    put(key, PrimitiveCodec.encodeVarLong(1)).whenComplete((r1, e1) -> {
                        if (e1 == null) {
                            future.complete(1L);
                        } else {
                    ...
                    put(key, PrimitiveCodec.encodeVarLong(value)).whenComplete((r1, e1) -> {
                        if (e1 == null) {
        ...
        future.thenAcceptAsync(v -> onWrite(key, PrimitiveCodec.encodeVarLong(v), stack), executorService);
        future.join();
        return future;
    }
