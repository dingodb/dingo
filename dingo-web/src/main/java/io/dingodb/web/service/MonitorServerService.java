...
public class MonitorServerService {
        ...
        return rootMetaServiceClient.getSubMetaServices().entrySet().stream().map(e -> {
            ...
            List<TreeTable> tables = tableMap.keySet().stream()
                .map(table -> {
                    ...
                    return new TreeTable(table, tableId, indices);
                })
                .collect(Collectors.toList());
            return new TreeSchema(schemaName, tables);
    ...
    public ClusterInfo getClusterResource(String key) {
        ...
        promMetricService.loadMetric(currentTimeSeconds);
        ...
        AtomicInteger storeIndex = new AtomicInteger(0);
        List<StoreInfo> indexInfoList = storeList
            .stream()
            .filter(store -> store.storeType() == 1)
            .map(store -> {
                StoreRegions storeRegions = getIndexRegionsInfo(store.serverLocation());
                ...
                ProcessInfo processInfo = promMetricService.getProcessMetric(store.serverLocation().toString());
                boolean exceedAlarm = exceedAlarm(resourceInfo);
                storeIndex.incrementAndGet();
                return new StoreInfo(store.id(), "index" + storeIndex.get(),
                    store.serverLocation().getHost(), exceedAlarm, resourceInfo, processInfo, storeRegions);
            })
            .collect(Collectors.toList());
        ...
        List<StoreInfo> storeInfoList = storeList
            ...
            .map(store -> {
                StoreRegions storeRegions = getStoreRegionsInfo(store.serverLocation());
                String instance = store.serverLocation().getHost() + ":" + nodeExportPort;
                ResourceInfo resourceInfo = nodeExporterMap.computeIfAbsent(instance,
                    s -> promMetricService.getResource(instance, currentTimeSeconds));
                ProcessInfo processInfo = promMetricService.getProcessMetric(store.serverLocation().toString());
                ...
                return new StoreInfo(store.id(), "store" + index.get(),
                    store.serverLocation().getHost(), exceedAlarm, resourceInfo, processInfo, storeRegions);
            })
            .collect(Collectors.toList());
    ...
    public static StoreRegions getIndexRegionsInfo(Location location) {
        ...
        Index.HelloRequest helloRequest = Index.HelloRequest.newBuilder()
            .setContext(context)
            .setGetRegionMetrics(true)
            .build();
        Index.HelloResponse response = indexServiceConnector.exec(stub -> stub.hello(helloRequest));
        return new StoreRegions(response.getRegionCount(), response.getRegionLeaderCount());
    }

    public static StoreRegions getStoreRegionsInfo(Location location) {
        ...
        Store.HelloRequest helloRequest = Store.HelloRequest.newBuilder()
            ...
            .setGetRegionMetrics(true)
            .build();
        Store.HelloResponse response = storeServiceConnector.exec(stub -> stub.hello(helloRequest));
        return new StoreRegions(response.getRegionCount(), response.getRegionLeaderCount());
    }
    ...
    private void transformRegion(TableDefinition tableDefinition,
        ...
        regionDtoList.add(new Region(
            ...
            StringUtils.join(rangeDistribution.getVoters()), range));
    }
    ...
    public RegionDetailInfo getRegion(long regionId) {
        ...
        String deleteTime = df.format(new Date(region.deleteTime()));
        return new RegionDetailInfo(type, createTime, deleteTime);
    }
