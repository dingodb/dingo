...
public interface TransactionServiceProvider {
    ...
    class Impl {
        ...
        private Impl() {
            Iterator<TransactionServiceProvider> iterator
                = ServiceLoader.load(TransactionServiceProvider.class).iterator();
            this.serviceProvider = iterator.next();
            if (iterator.hasNext()) {
                LogUtils.warn(log, "Load multi Transaction service provider, use {}.", serviceProvider.getClass().getName());
            }
        }
    }
