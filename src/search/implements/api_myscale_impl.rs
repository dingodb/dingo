...
pub fn bm25_search(
    ...
    let query_executor: QueryExecutor<'_, Vec<RowIdWithScore>> = QueryExecutor::new(&sentence_query);

    // Compute query results.
    let result: Vec<RowIdWithScore> = query_executor.execute(&index_reader_bridge.reader.searcher()).map_err(|e|{
        ERROR!(function:"bm25_search", "{}", e);
