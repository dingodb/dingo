...
impl Drop for IndexReaderBridge {
    fn drop(&mut self) {
        INFO!("IndexReaderBridge has been dropped. index_path:[{}]", self.path);
    }
}
...
#[cfg(test)]
mod tests{
    use tantivy::{merge_policy::LogMergePolicy, query::QueryParser, schema::{Schema, FAST, INDEXED, STORED, TEXT}, Document, Index, IndexWriter, ReloadPolicy, Term};
    use tempfile::TempDir;

    use crate::search::{bridge::index_reader_bridge::IndexReaderBridge, collector::row_id_bitmap_collector::RowIdRoaringCollector};

    fn index_some_docs_in_temp_directory(index_directory_str: &str)-> (IndexReaderBridge, IndexWriter) {
        ...
        let mut writer = index.writer_with_num_threads(2, 1024 * 1024 * 64).expect("Can't create index writer");
        ...
        let reader = index
            ...
            .try_into().expect("Can't set reload policy");
    ...
    fn test_get_reader_address() {
        ...
        let temp_directory_str_1 = temp_directory_1.path().to_str().expect("Can't get temp directory str");
        let temp_directory_str_2 = temp_directory_2.path().to_str().expect("Can't get temp directory str");
        ...
        assert_ne!(index_reader_bridge_1.reader_address(), index_reader_bridge_2.reader_address());
    }

    #[test]
    fn test_reload_reader() {
        let temp_directory = TempDir::new().expect("Can't create temp directory");
        let temp_directory_str = temp_directory.path().to_str().expect("Can't get temp directory str");
        let (index_reader_bridge, mut index_writer) = index_some_docs_in_temp_directory(temp_directory_str);

        // Init some variables for search.
        let text_field = index_reader_bridge.index.schema().get_field("text").unwrap();
        let row_id_field = index_reader_bridge.index.schema().get_field("row_id").unwrap();
        ...
        let reader = index_reader_bridge.index.reader().expect("Can't get reader from index");
        ...
        let searched_bitmap_3 = reader.searcher().search(&text_query, &row_id_collector).expect("Can't execute search.");
