...
mod tests {
    ...
    pub fn test_add_document_and_commit(){
        ...
        let searcher_1 = index_writer_bridge.index.reader().expect("Can't get reader from index").searcher();
        ...
        let searcher_2 = index_writer_bridge.index.reader().expect("Can't get reader from index").searcher();
    ...
    pub fn test_delete_terms() {
        ...
        // Initialize a temp `IndexWriterBridge` for test.
        let index_writer_bridge = create_index_in_temp_directory(directory_str);
        let row_id_field: tantivy::schema::Field = index_writer_bridge.index.schema().get_field("row_id").expect("Can't get `row_id` field.");
        ...
        let searcher_3 = index_writer_bridge.index.reader().expect("Can't get reader from index").searcher();
