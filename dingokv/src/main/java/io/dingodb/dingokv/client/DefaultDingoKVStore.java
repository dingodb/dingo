...
import io.dingodb.dingokv.DescriberManager;
import io.dingodb.dingokv.FollowerStateListener;
import io.dingodb.dingokv.JRaftHelper;
import io.dingodb.dingokv.LeaderStateListener;
import io.dingodb.dingokv.RegionEngine;
import io.dingodb.dingokv.StateListener;
import io.dingodb.dingokv.StateListenerContainer;
import io.dingodb.dingokv.StoreEngine;
import io.dingodb.dingokv.client.failover.FailoverClosure;
import io.dingodb.dingokv.client.failover.ListRetryCallable;
import io.dingodb.dingokv.client.failover.RetryCallable;
import io.dingodb.dingokv.client.failover.RetryRunner;
import io.dingodb.dingokv.client.failover.impl.BoolFailoverFuture;
import io.dingodb.dingokv.client.failover.impl.FailoverClosureImpl;
import io.dingodb.dingokv.client.failover.impl.ListFailoverFuture;
import io.dingodb.dingokv.client.failover.impl.MapFailoverFuture;
import io.dingodb.dingokv.client.pd.FakePlacementDriverClient;
...
import io.dingodb.dingokv.errors.ApiExceptionHelper;
...
import io.dingodb.dingokv.metrics.KVMetricNames;
...
import io.dingodb.dingokv.storage.KVIterator;
import io.dingodb.dingokv.storage.KVStoreClosure;
import io.dingodb.dingokv.storage.NodeExecutor;
import io.dingodb.dingokv.storage.RawKVStore;
import io.dingodb.dingokv.storage.Sequence;
import io.dingodb.dingokv.storage.zip.ZipStrategyManager;
import io.dingodb.dingokv.util.ByteArray;
import io.dingodb.dingokv.util.Constants;
import io.dingodb.dingokv.util.Lists;
import io.dingodb.dingokv.util.StackTraceUtil;
import io.dingodb.dingokv.util.Strings;
import io.dingodb.dingokv.util.concurrent.AffinityNamedThreadFactory;
import io.dingodb.dingokv.util.concurrent.DistributedLock;
import io.dingodb.dingokv.util.concurrent.disruptor.Dispatcher;
import io.dingodb.dingokv.util.concurrent.disruptor.TaskDispatcher;
import io.dingodb.dingokv.util.concurrent.disruptor.WaitStrategyType;
...
public class DefaultDingoKVStore implements DingoKVStore {
    private static final Logger LOG = LoggerFactory.getLogger(DefaultDingoKVStore.class);
    ...
    private DingoKVStoreOptions opts;
    ...
    @Override
    public synchronized boolean init(final DingoKVStoreOptions opts) {
        if (this.started) {
            LOG.info("[DefaultDingoKVStore] already started.");
            return true;
        }
        ...
        if (!this.dingoKVRpcService.init(rpcOpts)) {
            LOG.error("Fail to init [DingoKVRpcService].");
            return false;
        }
        ...
        LOG.info("[DefaultDingoKVStore] start successfully, options: {}.", opts);
    ...
    public synchronized void shutdown() {
        ...
        if (this.dingoKVRpcService != null) {
            this.dingoKVRpcService.shutdown();
        }
        ...
        LOG.info("[DefaultDingoKVStore] shutdown successfully.");
    }
    ...
    private void internalGet(final byte[] key, final boolean readOnlySafe, final CompletableFuture<byte[]> future,
        ...
        if (regionEngine != null) {
            ...
            this.dingoKVRpcService.callAsyncWithRpc(request, closure, lastCause, requireLeader);
        }
    }
    ...
    private void internalContainsKey(final byte[] key, final CompletableFuture<Boolean> future,
        ...
        if (regionEngine != null) {
            ...
            request.setRegionId(region.getId());
            request.setRegionEpoch(region.getRegionEpoch());
            this.dingoKVRpcService.callAsyncWithRpc(request, closure, lastCause);
        }
    }
    ...
    @Override
    public DingoKVIterator<KVEntry> iterator(final byte[] startKey, final byte[] endKey, final int bufSize) {
        return iterator(startKey, endKey, bufSize, true);
    }

    @Override
    public DingoKVIterator<KVEntry> iterator(final String startKey, final String endKey, final int bufSize) {
        return iterator(startKey, endKey, bufSize, true);
    }
    ...
    public DingoKVIterator<KVEntry> iterator(final byte[] startKey, final byte[] endKey, final int bufSize,
                                             final boolean readOnlySafe, final boolean returnValue) {
        return new DefaultDingoKVIterator(this, startKey, endKey, bufSize, readOnlySafe, returnValue);
    }
    ...
    private void checkState() {
        ...
        if (!this.started) {
            throw new DingoKVRuntimeException("dingo kv is not started or shutdown");
        }
    }
    ...
    private abstract static class Batching<T, E, F> {
