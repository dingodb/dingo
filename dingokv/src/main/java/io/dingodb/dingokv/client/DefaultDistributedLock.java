...
import io.dingodb.dingokv.errors.InvalidLockAcquirerException;
...
class DefaultDistributedLock extends DistributedLock<byte[]> {
    ...
    private final DefaultDingoKVStore dingoKVStore;
    ...
    protected DefaultDistributedLock(byte[] target, long lease, TimeUnit unit, ScheduledExecutorService watchdog,
                                     DefaultDingoKVStore dingoKVStore) {
        super(target, lease, unit, watchdog);
        this.dingoKVStore = dingoKVStore;
    }
    ...
    public void unlock() {
        ...
        try {
            final Owner owner = this.dingoKVStore.releaseLockWith(internalKey, acquirer).get();
            updateOwner(owner);
    ...
    protected Owner internalTryLock(final byte[] ctx) {
        ...
        final CompletableFuture<Owner> future = this.dingoKVStore.tryLockWith(internalKey, false, acquirer);
    ...
    private ScheduledFuture<?> scheduleKeepingLease(final ScheduledExecutorService watchdog, final byte[] key,
        ...
        return watchdog.scheduleAtFixedRate(() -> {
            try {
                ...
                this.dingoKVStore.tryLockWith(key, true, acquirer).whenComplete((result, throwable) -> {
                    if (throwable != null) {
