...
public class DefaultDingoKVIterator implements DingoKVIterator<KVEntry> {
    ...
    public DefaultDingoKVIterator(DefaultDingoKVStore dingoKVStore, byte[] startKey, byte[] endKey, int bufSize,
                                  boolean readOnlySafe, boolean returnValue) {
        this.dingoKVStore = dingoKVStore;
        this.pdClient = dingoKVStore.getPlacementDriverClient();
    ...
    public synchronized boolean hasNext() {
        if (this.buf.isEmpty()) {
            while (this.endKey == null || BytesUtil.compare(this.cursorKey, this.endKey) < 0) {
                final List<KVEntry> kvEntries = this.dingoKVStore.singleRegionScan(this.cursorKey, this.endKey,
