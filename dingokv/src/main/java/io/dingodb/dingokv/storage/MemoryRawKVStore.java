...
import io.dingodb.dingokv.util.RegionHelper;
...
public class MemoryRawKVStore extends BatchRawKVStore<MemoryDBOptions> {
    ...
    void doSnapshotSave(final MemoryKVStoreSnapshotFile snapshotFile, final String snapshotPath, final Region region)
        ...
        try {
            ...
            snapshotFile
                .writeToFile(tempPath, FENCING_KEY_DB, new MemoryKVStoreSnapshotFile.FencingKeyDB(subRangeMap(this.fencingKeyDB, region)));
    ...
    void doSnapshotLoad(final MemoryKVStoreSnapshotFile snapshotFile, final String snapshotPath) throws Exception {
        ...
        try {
            final MemoryKVStoreSnapshotFile.SequenceDB sequenceDB = snapshotFile.readFromFile(snapshotPath, SEQUENCE_DB, MemoryKVStoreSnapshotFile.SequenceDB.class);
            final MemoryKVStoreSnapshotFile.FencingKeyDB fencingKeyDB = snapshotFile.readFromFile(snapshotPath, FENCING_KEY_DB,
                MemoryKVStoreSnapshotFile.FencingKeyDB.class);
            final MemoryKVStoreSnapshotFile.LockerDB lockerDB = snapshotFile.readFromFile(snapshotPath, LOCKER_DB, MemoryKVStoreSnapshotFile.LockerDB.class);

            this.sequenceDB.putAll(sequenceDB.data());
            this.fencingKeyDB.putAll(fencingKeyDB.data());
            this.lockerDB.putAll(lockerDB.data());

            final MemoryKVStoreSnapshotFile.TailIndex tailIndex = snapshotFile.readFromFile(snapshotPath, TAIL_INDEX, MemoryKVStoreSnapshotFile.TailIndex.class);
            final int tail = tailIndex.data();
            final List<MemoryKVStoreSnapshotFile.Segment> segments = Lists.newArrayListWithCapacity(tail + 1);
            for (int i = 0; i <= tail; i++) {
                final MemoryKVStoreSnapshotFile.Segment segment = snapshotFile.readFromFile(snapshotPath, SEGMENT + i, MemoryKVStoreSnapshotFile.Segment.class);
                segments.add(segment);
            }
            for (final MemoryKVStoreSnapshotFile.Segment segment : segments) {
