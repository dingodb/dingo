...
public class ExecutorApi implements io.dingodb.server.api.ExecutorApi {
    ...
    public boolean insert(CommonId tableId, Object[] row) {
        return storeService.getInstance(tableId).insert(row);
    }

    @Override
    public boolean update(CommonId tableId, Object[] row) {
        return storeService.getInstance(tableId).update(row);
    }

    @Override
    public boolean delete(CommonId tableId, Object[] row) {
        return storeService.getInstance(tableId).delete(row);
    }

    @Override
    public List<Object[]> select(CommonId tableId, Object[] row, boolean[] hasData) {
        return storeService.getInstance(tableId).select(row, hasData);
    }
}
