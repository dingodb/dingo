...
import io.dingodb.store.row.LeaderStateListener;
import io.dingodb.store.row.RegionEngine;
import io.dingodb.store.row.StateListener;
import io.dingodb.store.row.StateListenerContainer;
...
import io.dingodb.store.row.client.failover.impl.ListFailoverFuture;
import io.dingodb.store.row.client.failover.impl.MapFailoverFuture;
import io.dingodb.store.row.client.pd.FakePlacementDriverClient;
...
import io.dingodb.store.row.storage.KVIterator;
...
import io.dingodb.store.row.util.concurrent.AffinityNamedThreadFactory;
import io.dingodb.store.row.util.concurrent.DistributedLock;
import io.dingodb.store.row.util.concurrent.disruptor.Dispatcher;
import io.dingodb.store.row.util.concurrent.disruptor.TaskDispatcher;
import io.dingodb.store.row.util.concurrent.disruptor.WaitStrategyType;
...
public class DefaultDingoRowStore implements DingoRowStore {
    private static final Logger LOG = LoggerFactory.getLogger(DefaultDingoRowStore.class);
    ...
    private DingoRowStoreOptions opts;
    ...
    public synchronized void shutdown() {
        ...
        if (this.dingoRowStoreRpcService != null) {
            this.dingoRowStoreRpcService.shutdown();
        }
        ...
        LOG.info("[DefaultDingoRowStore] shutdown successfully.");
    }
    ...
    @Override
    public DingoRowStoreIterator<KVEntry> iterator(final byte[] startKey, final byte[] endKey, final int bufSize) {
        return iterator(startKey, endKey, bufSize, true);
    }

    @Override
    public DingoRowStoreIterator<KVEntry> iterator(final String startKey, final String endKey, final int bufSize) {
        return iterator(startKey, endKey, bufSize, true);
    }
    ...
    public DingoRowStoreIterator<KVEntry> iterator(final byte[] startKey, final byte[] endKey, final int bufSize,
                                                   final boolean readOnlySafe, final boolean returnValue) {
        return new DefaultDingoRowStoreIterator(this, startKey, endKey, bufSize, readOnlySafe, returnValue);
    }
    ...
    private void checkState() {
        ...
        if (!this.started) {
            throw new DingoRowStoreRuntimeException("dingo kv is not started or shutdown");
        }
    }
