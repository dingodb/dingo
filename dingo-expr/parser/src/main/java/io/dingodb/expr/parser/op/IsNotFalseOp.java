...
public final class IsNotFalseOp extends Op {
    private IsNotFalseOp(OpType type) {
        super(type);
        name = "IS_FALSE";
    }

    @Nonnull
    public static IsNotFalseOp fun() {
        return new IsNotFalseOp(OpType.FUN);
    }

    @Nonnull
    @Override
    protected RtIsNotFalse createRtOp(RtExpr[] rtExprArray) throws FailGetEvaluator {
