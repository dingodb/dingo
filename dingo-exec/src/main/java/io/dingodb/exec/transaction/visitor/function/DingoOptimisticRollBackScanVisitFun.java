...
public class DingoOptimisticRollBackScanVisitFun {

    public static Collection<Vertex> visit(
        Job job, IdGenerator idGenerator, Location currentLocation, ITransaction transaction,
                DingoTransactionRenderJob visitor, OptimisticRollBackScanLeaf optimisticRollBackScanLeaf) {
        ...
        for (Map.Entry<CommonId, Channel> channelEntry : channelMap.entrySet()) {
            ...
            OptimisticRollBackScanParam param = new OptimisticRollBackScanParam(
                dingoType,
                transaction.getJobSeqId()
            );
        ...
        OptimisticRollBackScanParam param = new OptimisticRollBackScanParam(dingoType, transaction.getJobSeqId());
        Vertex vertex = new Vertex(SCAN_CACHE, param);
        Task task = job.getOrCreate(
            currentLocation,
            idGenerator,
