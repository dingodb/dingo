...
public class DingoPessimisticResidualLockVisitFun {
    ...
    public static Collection<Vertex> visit(
        Job job, IdGenerator idGenerator, Location currentLocation, ITransaction transaction,
                DingoTransactionRenderJob visitor, PessimisticResidualLockLeaf pessimisticResidualLockLeaf) {
        Collection<Vertex> inputs = pessimisticResidualLockLeaf.getData().accept(visitor);
        List<Vertex> outputs = new ArrayList<>();
        for (Vertex input : inputs) {
            PessimisticResidualLockParam param = new PessimisticResidualLockParam(
                new BooleanType(true),
                transaction.getIsolationLevel(),
                transaction.getStartTs()
            );
            Vertex vertex = new Vertex(PESSIMISTIC_RESIDUAL_LOCK, param);
