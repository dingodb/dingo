...
public interface TenantServiceProvider {
    ...
    class Impl {
        ...
        public Impl() {
            Iterator<TenantServiceProvider> iterator = ServiceLoader.load(TenantServiceProvider.class).iterator();
            this.provider = iterator.next();
            if (iterator.hasNext()) {
                LogUtils.warn(log, "Load multi tenant service provider, use {}.", provider.getClass().getName());
            }
        }
    }
