...
import static io.dingodb.common.mysql.error.ErrorCode.ErrDupEntry;
...
@Slf4j
public abstract class AbstractFiller implements BackFiller {
    public Table table;
    protected long schemaId;
    protected IndexTable indexTable;
    protected boolean primaryOrUnique;
    ...
    public void duplicateKey(Object[] tuple) {
        ...
        if ("replicaTable".equalsIgnoreCase(name)) {
            name = table.getName();
        }
        throw DingoErrUtil.newStdErr(ErrDupEntry, duplicateKey, name);
    }

    public void initFiller() {
        schemaId = table.getTableId().domain;
        primaryOrUnique = indexTable.unique;
    }

    public abstract void validate(Object[] tuples);
