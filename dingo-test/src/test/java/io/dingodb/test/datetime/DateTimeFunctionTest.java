...
package io.dingodb.test.datetime;
...
public class DateTimeFunctionTest {
    ...
    private static Stream<Arguments> getArguments() {
        return Stream.of(
            ...
            // datetime_format
            arguments("select datetime_format('1999/1/01 01:01:01', '%Y/%m/%d %T')", "1999/01/01 01:01:01"),
            arguments("select datetime_format('1999.01.01 01:01:01', '%Y.%m.%d %T')", "1999.01.01 01:01:01"),
            arguments("select datetime_format('19990101010101', '%Y%m%d %T')", "19990101 01:01:01"),
            arguments("select datetime_format('20220413103706','%Y/%m/%d %H:%i:%S')", "2022/04/13 10:37:06"),
            arguments("select datetime_format('2022-04-13 10:37:26', '%Ss')", "26s"),
            arguments(
                "select datetime_format('1999-01-01 10:37:26', '%Y year %m month %d day and %s seconds')",
                "1999 year 01 month 01 day and 26 seconds"
            ),
            arguments(
                "select datetime_format('2022-04-13 10:37:26', '%m mon %d day %Y year, %H hour %i min %S sec')",
                "04 mon 13 day 2022 year, 10 hour 37 min 26 sec"
            ),
            arguments("select datetime_format('2022-04-13 10:37:36', '%H:%i:%S')", "10:37:36"),
            arguments(
                "select datetime_format('20220413103726', 'Year:%Y Month:%m Day:%d')",
            ...
            // datetime_format w/ cast
            arguments(
                "SELECT DATETIME_FORMAT(CAST('2020.11.30 9:1:1' AS TIMESTAMP), '%Y year, %m month')",
                "2020 year, 11 month"
            ),
            arguments(
                "SELECT DATETIME_FORMAT(CAST('2020.11.30 01:1:01' AS TIMESTAMP), '%Y year, %m month')",
                "2020 year, 11 month"
            ),
            arguments(
                "SELECT DATETIME_FORMAT(CAST('2020/11/30 01:1:01' AS TIMESTAMP), '%Y year, %m month')",
    ...
    public void testNow(String fun) throws SQLException {
        ...
        assertThat((Timestamp) sqlHelper.querySimpleValue(sql))
    ...
    public void testCurDate(String fun) throws SQLException {
        String sql = "select " + fun;
        assertThat(sqlHelper.querySimpleValue(sql).toString())
    ...
    public void testCurTime(String fun) throws SQLException {
        String sql = "select " + fun;
        assertThat((Time) sqlHelper.querySimpleValue(sql))
            .isCloseTo(DateTimeUtils.currentTime(), 3000L);
    }
    ...
    public void testUnixTimeStamp(String dateTime, String result) throws SQLException {
        ...
        assertThat(sqlHelper.querySimpleValue(sql)).isEqualTo(Timestamp.valueOf(result).getTime());
    }
    ...
    public void testUnixTimeStamp1(long timestamp) throws SQLException {
        ...
        assertThat(sqlHelper.querySimpleValue(sql)).isEqualTo(timestamp);
    }
    ...
    public void testUnixTimeStamp2(String expr) throws SQLException {
        ...
        assertThat((long) sqlHelper.querySimpleValue(sql))
            .isCloseTo(Timestamp.valueOf(LocalDateTime.now()).getTime(), Offset.offset(3000L));
    }

    @ParameterizedTest
    @MethodSource("getArguments")
    public void testDateTimeFormat(String sql, String result) throws SQLException {
        assertThat(sqlHelper.querySimpleValue(sql)).isEqualTo(result);
    }
    ...
    public void testFromUnixTime(long timestamp) throws SQLException {
        ...
        assertThat(((Timestamp) sqlHelper.querySimpleValue(sql)).getTime()).isEqualTo(timestamp);
    }
}
